...
class PluginsAlpha_OpenAI {
    ...
    private static function cfg() : array {
        if (!class_exists('PluginsAlpha_Settings')) {
            return [
                'key'          => trim(get_option('alpha_gpt_posts_openai_key', '')),
                'model_text'   => get_option('alpha_gpt_posts_model_text', 'gpt-4o-mini'),
                'model_image'  => get_option('alpha_gpt_posts_model_image', 'gpt-image-1'),
                'temperature'  => floatval(get_option('alpha_gpt_posts_temperature', 0.6)),
                'max_tokens'   => intval(get_option('alpha_gpt_posts_max_tokens', 6000)),
                'timeout'      => 60,
            ];
        }
