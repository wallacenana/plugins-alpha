...
final class PluginsAlpha_WS_CPT
{
    ...
    public static function register(): void
    {
        ...
        register_post_type(self::POST_TYPE, [
            'labels' => [
                'name'          => __('Web Stories', 'plugins-alpha'),
                'singular_name' => __('Web Story', 'plugins-alpha'),
                'add_new'       => __('Adicionar', 'plugins-alpha'),
                'add_new_item'  => __('Adicionar Web Story', 'plugins-alpha'),
                'edit_item'     => __('Editar Web Story', 'plugins-alpha'),
            ],
    ...
    public static function row_actions(array $actions, \WP_Post $post): array
    {
        ...
        // abre no builder
        $url = admin_url('admin.php?page=plugins-alpha-ws-generator&story_id=' . (int)$post->ID);
        $actions['pga_ws_open'] = '<a href="' . esc_url($url) . '">' . esc_html__('Abrir no Builder', 'plugins-alpha') . '</a>';
    ...
    public static function admin_redirects(): void
    {
        ...
        // 1) Add new padrão do WP: post-new.php?post_type=ws_generator
        if (isset($_GET['post_type']) && $_GET['post_type'] === $pt && strpos($_SERVER['REQUEST_URI'] ?? '', 'post-new.php') !== false) {
            $url = admin_url('admin.php?page=plugins-alpha-ws-generator'); // sem story_id (novo)
            wp_safe_redirect($url);
            exit;
        }

        // 2) Edit padrão do WP: post.php?post=123&action=edit
        if (isset($_GET['post'], $_GET['action']) && $_GET['action'] === 'edit') {
            $post_id = absint($_GET['post']);
            if ($post_id) {
                $post = get_post($post_id);
                if ($post && $post->post_type === $pt) {
                    $url = admin_url('admin.php?page=plugins-alpha-ws-generator&story_id=' . $post_id);
                    wp_safe_redirect($url);
                    exit;
                }
            }
    ...
    public static function render(): void
    {
        ...
        if (!$story_id && !empty($_GET['page']) && is_string($_GET['page'])) {
            ...
            if (strpos($page, 'story_id=') !== false) {
                parse_str(parse_url('http://x/?' . ltrim(strstr($page, '?') ?: '', '?'), PHP_URL_QUERY), $tmp);
        ...
        if ($story_id > 0) {
            if ($story_status === 'publish') {
                $status_label = esc_html__('Publicado', 'plugins-alpha');
                $status_class = 'is-publish';
            } elseif ($story_status === 'future') {
                $status_label = esc_html__('Agendado', 'plugins-alpha');
                $status_class = 'is-future';
            } elseif ($story_status === 'trash') {
                $status_label = esc_html__('Excluido', 'plugins-alpha');
                $status_class = 'is-trash';
            } else {
                $status_label = esc_html__('Rascunho', 'plugins-alpha');
        ...
        $back = admin_url('admin.php?page=plugins-alpha-ws-generator');
            ...
            <div class="wrap pga-layout">
                <div class="pga-header-fixed">
                    <div class="pga-header-col pga-a-center">
                        <div>
                            <h1><?php esc_html_e('Gerador — WS Generator', 'plugins-alpha'); ?></h1>
                            <p class="pga-descricao"><?php esc_html_e('Criação de Web Stories com IA', 'plugins-alpha'); ?></p>
                        </div>
                    </div>
                    ...
                    <div class="pga-header-col pga-a-center pga-header-center">
                        <div class="pga-story-center">
                            <span id="pga_story_title_header" class="pga-post-title">' . esc_html($story_title ?: __('Story sem título', 'plugins-alpha')) . '</span>
                        </div>
                    </div>' : '';
                    ?>
                    ...
                    <div class="pga-header-col pga-a-center">
                        ...
                        <?php
                        $label_text = esc_html__('Publicar', 'plugins-alpha');
                        $label_html = '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-sparkles h-4 w-4 mr-2"><path d="M9.937 15.5A2 2 0 0 0 8.5 14.063l-6.135-1.582a.5.5 0 0 1 0-.962L8.5 9.936A2 2 0 0 0 9.937 8.5l1.582-6.135a.5.5 0 0 1 .963 0L14.063 8.5A2 2 0 0 0 15.5 9.937l6.135 1.581a.5.5 0 0 1 0 .964L15.5 14.063a2 2 0 0 0-1.437 1.437l-1.582 6.135a.5.5 0 0 1-.963 0z"></path><path d="M20 3v4"></path><path d="M22 5h-4"></path><path d="M4 17v2"></path><path d="M5 18H3"></path></svg> '
                            . esc_html__('Gerar Story', 'plugins-alpha');
                ...
                <main class="pga-main">
                    <div id="pga_tabs" class="pga-ws-theme-tabs">
                        <label class="pga-ws-style">
                            ...
                            <span><?php esc_html_e('Normal', 'plugins-alpha'); ?></span>
                        </label>
                        <label class="pga-ws-style">
                            <input type="radio" name="pga_ws_theme" value="theme-news">
                            <span><?php esc_html_e('Newsroom', 'plugins-alpha'); ?></span>
                        </label>
                        <label class="pga-ws-style">
                            <input type="radio" name="pga_ws_theme" value="theme-dark">
                            <span><?php esc_html_e('Dark Neon', 'plugins-alpha'); ?></span>
                        </label>
                        <label class="pga-ws-style">
                            <input type="radio" name="pga_ws_theme" value="theme-soft">
                            <span><?php esc_html_e('Soft Clean', 'plugins-alpha'); ?></span>
                        </label>
                        <label class="pga-ws-style">
                            <input type="radio" name="pga_ws_theme" value="theme-pop">
                            <span><?php esc_html_e('Pop Bold', 'plugins-alpha'); ?></span>
                        </label>
                    </div>
                            ...
                            <div class="pga-modal__body">
                                ...
                                <div class="pga-field">
                                    ...
                                    <select id="pga_ws_categories"
                                        data-placeholder="<?php echo esc_attr__('Selecione categorias...', 'plugins-alpha'); ?>">
                                    ...
                                    <div class="pga-field">
                                        ...
                                        <div class="pga-media">
                                            ...
                                            <div class="pga-media__btns">
                                                <button type="button" class="button" id="pga_ws_pick_logo"><?php esc_html_e('Selecionar', 'plugins-alpha'); ?></button>
                                                <button type="button" class="button" id="pga_ws_clear_logo"><?php esc_html_e('Remover', 'plugins-alpha'); ?></button>
                                            </div>
                                        </div>
                                ...
                                <?php if (!$story_id) { ?>
                                    ...
                                    <div class="pga-field">
                                        ...
                                        <select id="pga_ws_locale" class="pga-select">
                                            ...
                                            <option value="en_US" <?php selected($modal_locale, 'en_US'); ?>>English (US)</option>
                                            <option value="es_ES" <?php selected($modal_locale, 'es_ES'); ?>>Español (ES)</option>
                                        </select>
                                    </div>

                                    <div class="pga-field">
                                        <label><?php esc_html_e('Início das Postagens', 'plugins-alpha'); ?></label>
                                    ...
                                    <div id="multi-selection">
                                        <div class="pga-field">
                                            ...
                                            <select id="pga_ws_posts_multi"
                                                ...
                                                multiple="multiple"
                                                data-placeholder="<?php echo esc_attr__('Buscar postagens...', 'plugins-alpha'); ?>"
                                                style="width:100%;">
                                                <?php if (!empty($orion_posts)) : ?>
                                                    <?php foreach ($orion_posts as $p) : ?>
                                                        <option value="<?php echo esc_attr($p->ID); ?>" <?php selected($source_id, (int)$p->ID); ?>>
