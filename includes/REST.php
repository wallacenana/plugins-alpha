...
class PluginsAlpha_REST {
    ...
    private static function get_opt($key, $default=[]) {
        ...
        if ($opt === null) { update_option($key, $default, false); return $default; }
        return is_array($opt) ? $opt : $default;
    }
    private static function set_opt($key, $val){ update_option($key, is_array($val)? array_values($val) : $val, false); }

    private static function kw_get_pending(): array { return self::get_opt('pga_keywords_pending', []); }
    private static function kw_get_done(): array    { return self::get_opt('pga_keywords_done', []); }
    private static function kw_set_pending(array $a){ self::set_opt('pga_keywords_pending', self::unique_clean($a)); }
    private static function kw_clear_pending()      { self::set_opt('pga_keywords_pending', []); }
    private static function kw_clear_done()         { self::set_opt('pga_keywords_done', []); }
    private static function kw_move_to_done_one(string $used){
        ...
        $exists = false;
        foreach ($done as $d) {
            if ($lower($d) === $usedL) { $exists = true; break; }
        }
        if (!$exists) {
            $done[] = $used;
   ...
   private static function unique_clean(array $arr) : array {
    ...
    private static function clean($s){ return sanitize_text_field((string)$s); }
    ...
    public static function generate_single($req){
        ...
        return self::guard(function() use ($req){
            ...
            $res = PluginsAlpha_Pages_Generator::generate_and_insert($args, class_exists('PluginsAlpha_Settings')? PluginsAlpha_Settings::get() : []);
            if (is_wp_error($res)) {
                ...
                // devolve postId do “rascunho-inicial” se existir, para a UI poder linkar
                $pId = is_array($edata) && !empty($edata['post_id']) ? (int)$edata['post_id'] : 0;
    
                return new WP_Error($res->get_error_code() ?: 'pga_generate', $msg, ['status'=>$status, 'postId'=>$pId]);
            }
